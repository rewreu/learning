...
class TDNN2(nn.Module):
    def __init__(self, context: list, input_channels: int, output_channels: int, full_context: bool = True):
        ...
        if full_context:
            kernel_size = context[-1] - context[0] + 1 if len(context) > 1 else 1
            self.temporal_conv = nn.Conv1d(input_channels, output_channels, kernel_size)
        else:
            # use dilation
            delta = context[1] - context[0]
            self.temporal_conv = nn.Conv1d(input_channels, output_channels, kernel_size=len(context), dilation=delta)
...
if __name__ == "__main__":
    previous_layer_n_feat = 7
    layer_context = [-2, 0, 2]
    input_n_feat = previous_layer_n_feat
    tddn_layer = TDNN2(context=layer_context, input_channels=input_n_feat, output_channels=512, full_context=False)

    # Run a forward pass; batch.size = [BATCH_SIZE, INPUT_CHANNELS, SEQUENCE_LENGTH]
    batch = torch.rand(20, 7, 100)
    out = tddn_layer(batch)